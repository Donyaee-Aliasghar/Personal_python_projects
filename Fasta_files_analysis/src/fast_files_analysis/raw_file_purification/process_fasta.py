...
def pf():
    ...
    allowed_bases: set[str] = set("ATGC")
    min_len = 80
    seen_hashes = set()
    total_seqs = 0
    removed_short = 0
    ...
    with open(MAIN_RAW_FILE, "r", encoding="utf-8") as fin:
        with open(f"{PS_OUTPUT_DIR}/pure_sequence.fasta", "w", encoding="utf-8") as fout:
            with open(f"{FFPD_LOG_DIR}/fasta_file_purification_details.log", "w", encoding="utf-8") as flog:
                header = None
                seq_chunks = []
                ...
                for line in fin:
                    ...
                    if not line:
                        continue
                    if line.startswith(">"):
                        # اگر هدر قبلی وجود دارد، توالی قبلی را پردازش کن
                        if header is not None:
                            ...
                            if len(cleaned_seq) < min_len:
                                ...
                                if seq_hash in seen_hashes:
                                    ...
                                    kept_seqs += 1
                        # هدر جدید را مقداردهی کن و seq_chunks را ریست کن
                        header = line.split()[0]
                        seq_chunks = []
                    else:
                        seq_chunks.append(line)

                # در انتهای فایل، آخرین توالی را پردازش کن
                ...
                print(
                    "[✅] Cleaning done. Check log file for details.(results > logs > fasta_file_purification_details.log)"
                )
